...
- [x] **Step 1: Record runtime versions**

Create `.nvmrc` containing `24` and `.python-version` containing `3.13`.

- [x] **Step 2: Add repository-wide editor and ignore rules**
...
- [x] **Step 3: Add the local PostgreSQL contract**
...
- [x] **Step 4: Validate the repository foundation**
...
- [x] **Step 1: Install the managed Python runtime and dependency manager**
...
- [x] **Step 2: Create the backend manifest and lock dependencies**
...
- [x] **Step 4: Run the test and confirm it fails for the missing application**

Run: `cd backend && uv run pytest tests/test_health.py -v`

Expected: collection fails because `app.main` does not exist.

- [x] **Step 5: Implement settings and the health endpoint**
...
- [x] **Step 6: Add safe configuration examples and backend commands**
...
- [x] **Step 1: Install and select Node.js 24**

Source the existing nvm installation, install Node 24, run `nvm use 24`, and confirm `node --version` begins with `v24`.

- [x] **Step 2: Scaffold the Vite React TypeScript project and install quality dependencies**
...
- [x] **Step 3: Write the failing startup page test**
...
- [x] **Step 4: Run the test and confirm it fails before the page exists**

Run: `cd frontend && npm test`

Expected: test fails because the expected heading and status text are absent.

- [x] **Step 5: Implement the startup page and accessible base styling**
...
- [x] **Step 6: Configure tests and quality scripts**
...
- [x] **Step 7: Add a safe frontend environment example**
...
- [x] **Step 3: Replace the root README**
...
- [x] **Step 4: Verify documentation commands and the integrated setup**
...
Execution note: the sandbox denied loopback socket creation, so live `curl` verification was unavailable. The real FastAPI ASGI integration test verified the same route and exact JSON contract. Docker was not installed, so the Compose file received YAML syntax validation but not Docker runtime validation.

- [x] **Step 5: Check repository scope and commit**
